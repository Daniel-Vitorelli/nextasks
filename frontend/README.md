...
- Regras (definidas com o usuário): **bloco confirmado** completa a entidade conectada quando **todas** as conexões dela estiverem satisfeitas; **entidade concluída** auto-confirma os blocos conectados no período atual (checklist `"true"`, nota `"10"`); **desmarcar/bloco cancelado propaga em reverso** (reabre entidades que foram concluídas apenas por aquele bloco); `requiredCount` conta as confirmações históricas do bloco que satisfazem o filtro; ciclos são seguros.
...
- A UI do popover reflete a recorrência real do bloco: mostra "Todos os dias"/"Toda {dia}" no subtítulo das linhas e no header da visão bloco→entidade, só oferece o weekday do bloco (semanal), pré-seleciona a próxima data do dia correto ao escolher "Data específica", valida a data escolhida no fuso local e oferece correção com um clique para filtros legados impossíveis; blocos sem modo de confirmação aparecem desabilitados. Tem busca por título e hint contextual por visão (entidade × bloco).
- Os botões de gatilho (dialog de tarefa, nó de sub-tarefa, popover do evento) mostram um **badge** com o número de conexões, verde quando todas estão satisfeitas (`ConnectionBadge`).
- Trocar a confirmação de um bloco para `"none"` com conexões ativas é recusado (400) — criaria conexões impossíveis de satisfazer.
- A auto-confirmação é **create-if-missing** (upsert com `update: {}`): nunca sobrescreve uma confirmação explícita do período (o usuário desmarcou o bloco hoje? continua desmarcado; decisão explícita sempre vence e reabre entidades em cascata); só acontece se o bloco **já começou** no período atual e respeita o `dayFilter`; e a propagação só acontece para entidades que **de fato transicionaram** (pendente → concluída), então repetir confirmações não infla contagens nem cria linhas novas.
- A propagação é **reversível**: cada linha de `TimeBlockCompletion` registra `source` (`"explicit"`|`"auto"`) e `sourceEntityId` (chave `task:<id>`/`subtask:<id>`). Desmarcar um bloco (checklist `"false"` no período) roda `reversePropagateForBlock`: remove as auto-confirmações do bloco no período e reabre as entidades que só estavam satisfeitas por elas. Reabrir uma entidade (`done: false`) roda `reversePropagateForEntities`: apaga suas auto-confirmações no período e reavalia os blocos conectados. Excluir tarefa/sub-tarefa também reavalia os blocos da conexão removida (`removedBlockIds`).
- A propagação roda também no `POST` e `PATCH` de conexões: conectar uma entidade a um bloco já satisfeito (ex.: confirmado no histórico) completa a entidade na hora e confirma os blocos conectados no período atual; conexões novas de blocos que não começaram, de rotinas inativas ou com contagem inalcançável (filtro `date:` com `requiredCount > 1`) são recusadas (400).
- A lógica vive em `lib/server/connections.ts` (`countConfirmed`, `confirmBlockForConnection`, `confirmBlocksForDoneEntities`, `completeEntitiesForConnections`, `completeEntitiesForBlock`, `confirmationMatchesDayFilter`, `toConnectionRow`, `loadCompletionsByBlock`, `reversePropagate*`) e é chamada dentro das transações de `time-blocks/[id]/complete`, `connections` (POST/PATCH/DELETE), `tasks/[id]` (PATCH/DELETE) e `subtasks/[id]` (PATCH/DELETE). A cascata de conclusão de árvore compartilhada está em `lib/server/subtask-cascade.ts` (`markTaskDoneCascade`/`markSubtaskDoneCascade`), retornando apenas as entidades que transicionaram (`completedSubtaskIds`, `taskCompleted`/`completedTask`). O DELETE de conexão reavalia a entidade da conexão removida (remover o último obstáculo insatisfeito completa a entidade quando as restantes já estão satisfeitas).
...
- UI: `ConnectionPopover` abre em três lugares — detalhes da tarefa (`task-details-dialog`), por sub-tarefa na árvore (`subtask-node`) e no painel do bloco no calendário (`event-detail-popover`, botão no header). O estado global fica no `ConnectionsProvider` (montado no layout `[app]`). A sincronização entre telas usa o barramento de eventos tipado `lib/client/data-events.ts` (`useDataSync` registra o recarregamento de cada hook em canais `tasks`/`subtasks`/`connections`/`routines`/`time-blocks`/`progress`/`current-block`; cada mutação chama `notifyDataChanged(...)` com os canais afetados) — assim blocos concluídos server-side, reversões e mudanças de rotina ativa refletem em todas as telas abertas sem depender de eventos globais de janela.
